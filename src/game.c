...
receive_treasure_state ()
{
  ...
  uint32_t tamanho;
  memcpy(&tamanho_pkg, tamanho_pkg.data, sizeof(uint32_t));
  ...
  {
    perror("Erro ao acessar o sistema de arquivos com statvfs");
    prepare_ack_pkg(&jogo.send, get_seq_num(), ERROR);
    snail_send(&jogo.send);
    free(jogo.file_name);
    jogo.file_name = NULL;
  ...
  {
    fprintf(stderr, "Espaço insuficiente: %u bytes requeridos, %llu disponíveis.\n", 
  ...
  prepare_ack_pkg(&jogo.send, get_seq_num(), ACK);
  snail_send(&jogo.send);
